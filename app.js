...
const sessionSecret = process.env.SECRET || "wanderlust-dev-secret";
const port = process.env.PORT || 8080;

if (!process.env.ATLASDB_URL) {
  console.log("ATLASDB_URL not found. Falling back to local MongoDB: mongodb://127.0.0.1:27017/wanderlust");
}
...
main()
  ...
  .catch((err) => {
    console.error("MongoDB connection failed. Check ATLASDB_URL credentials or the local Mongo service.");
...
const store = MongoStore.create({
  mongoUrl: dbUrl,
  crypto: {
    secret: sessionSecret, //advance option..adding secret in form of crypto--fro encryption
  },
